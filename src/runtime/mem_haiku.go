...
func sysAllocOS(n uintptr, _ string) unsafe.Pointer {
	...
	if err != 0 {
		if int32(err) == _EACCES {
			print("runtime: mmap: access denied\n")
			exit(2)
		}
		if int32(err) == _EAGAIN {
...
func sysReserveOS(v unsafe.Pointer, n uintptr, _ string) unsafe.Pointer {
	p, err := mmap(v, n, _PROT_NONE, _MAP_ANON|_MAP_PRIVATE|_MAP_NORESERVE, -1, 0)
	if err != 0 {
		return nil
	}
	return p
...
func sysMapOS(v unsafe.Pointer, n uintptr, _ string) {
	// Haiku, like AIX, requires re-mapping rather than overlay; use
	// mprotect to upgrade a previously-reserved range to RW.
	_, err := mprotect(v, n, _PROT_READ|_PROT_WRITE)
	if int32(err) == _ENOMEM {
